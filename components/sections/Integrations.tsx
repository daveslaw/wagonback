...
export function Integrations() {
  return (
    <section id="integrations" className="py-20 md:py-28 bg-[#0d0d0d]">
      <div className="container mx-auto px-4 md:px-8 lg:px-16">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
          <h2 className="text-3xl sm:text-4xl md:text-5xl font-extralight tracking-wide uppercase text-white">
            Your Platforms,
            <br />
            <span className="text-white/40">Finally Talking.</span>
          </h2>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
          {platforms.map((platform) => (
            <div
              key={platform.name}
              className="group bg-[#161616] border border-white/5 rounded-xl px-4 py-4 hover:border-[#00c8ff]/20 hover:bg-[#161616]/80 transition-all duration-200 flex items-center justify-between"
            >
              <span className="text-sm font-light text-white/70 group-hover:text-white transition-colors">
                {platform.name}
              </span>
              <span className="text-[9px] tracking-wider uppercase text-white/20 group-hover:text-[#00c8ff]/50 transition-colors hidden sm:block">
                {platform.category}
              </span>
            </div>
          ))}
